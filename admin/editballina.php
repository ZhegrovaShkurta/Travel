...
{	
	...
	$maxsize = 10000000; 
		...
		mysqli_query($conn,"SET @id = ${ID_Ballina}");
		...
		$result = mysqli_query($conn,"CALL updateballina(@id, @tit, @per, @img, @emr)");
		if($result){
...
?>
<?php
$ID_Ballina = $_GET['ID_Ballina'];
$result = mysqli_query($conn,"SELECT * FROM ballina WHERE ID_Ballina=$ID_Ballina");
...
<center><div class="divlogin"><br>
        <h3  style="padding-left: 30px;">Ndrysho Te Dhenat</h3>
            <form method="post" enctype="multipart/form-data" action="editballina.php">
			...
			<input type="hidden" name="ID_Ballina" value='<?php echo $_GET['ID_Ballina'];?>' />
