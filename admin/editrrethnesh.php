...
{	
	$ID_RrethNesh = $_POST['ID_RrethNesh'];
	...
	$maxsize = 10000000; 
		...
		mysqli_query($conn,"SET @id = ${ID_RrethNesh}");
		...
		$result = mysqli_query($conn,"CALL updaterrethnesh(@id, @tit, @per, @img, @emr)");
		if($result){
...
?>
<?php
$ID_RrethNesh = $_GET['ID_RrethNesh'];
$result = mysqli_query($conn,"SELECT * FROM rrethnesh WHERE ID_RrethNesh=$ID_RrethNesh");
...
<a class="active" href="rrethnesh.php">Rreth Nesh</a>
...
<center><div class="divlogin"><br>
        <h3  style="padding-left: 30px;">Ndrysho Te Dhenat</h3>
            <form method="post" enctype="multipart/form-data" action="editrrethnesh.php">
			...
			<input type="hidden" name="ID_RrethNesh" value='<?php echo $_GET['ID_RrethNesh'];?>' />
