...
<?php
	...
	$sql = mysqli_query($conn,"SELECT * FROM biletat WHERE Titulli LIKE '%".$term."%' OR Pershkrimi LIKE '%".$term."%'  OR EmrF0 LIKE '%".$term."%'");  
	while($row = mysqli_fetch_array($sql)) { 		
			...
			echo "<td><button id='submit2'> <a style='color:white;' href=\"editbiletat.php?ID_Bileta=$row[ID_Bileta]\"><center>Ndrysho</a></button> <br> <button id='submit2'> <a style='color:white;' href=\"deletebiletat.php?ID_Bileta=$row[ID_Bileta]\" onClick=\"return confirm('Je i sigurte qe doni ta fshini?')\">Fshij</center></a></button></td>";
			echo "</tr>";
		}
